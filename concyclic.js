...
function test_concyclic_known() {
    const sides = tri_sides(g_tri);
    let test;
    test = concyclic_derived(g_tri, sides, 15, 16, "excentral");
    test = concyclic_derived(g_tri, sides, 13, 14, "excentral");
    test = concyclic_derived(g_tri, sides, 15, 16, "orthic");
    test = concyclic_derived(g_tri, sides, 13, 14, "brocard1");
    test = concyclic_derived(g_tri, sides, 13, 14, "brocard2");
    return 0;
}
...
function concyclic_derived3_low(tri,sides,k1,k2,k3,derived_str) {
    const Xk1 = get_Xn_cartesians(k1,tri,sides);
    const Xk2 = get_Xn_cartesians(k2,tri,sides);
    ...
    return [Xk1,Xk2,Xk3,Xk1_der,Xk2_der,Xk3_der];
}

const concyclic_derived3 = (tri,sides,k1,k2,k3,derived_str) => concyclicN(concyclic_derived3_low(tri,sides,k1,k2,k3,derived_str));
...
function concyclic_derived_triples(Xks) {
    ...
    const results = triples.map(p=>concyclic_derived_all3(g_tri,sides,p[0],p[1],p[2]))
    .flat(1)
    .sort((a,b) => (a.tri > b.tri) ? 1 : ((b.tri > a.tri) ? -1 : 0));
    return results;
}
...
function concyclic_derived4_low(tri,sides,k1,k2,k3,k4,derived_str) {
    const Xk1 = get_Xn_cartesians(k1,tri,sides);
    const Xk2 = get_Xn_cartesians(k2,tri,sides);
    const Xk3 = get_Xn_cartesians(k3,tri,sides);
    ...
    return [Xk1,Xk2,Xk3,Xk4,Xk1_der,Xk2_der,Xk3_der,Xk4_der];
}

const concyclic_derived4 = (tri,sides,k1,k2,k3,k4,derived_str) => concyclicN(concyclic_derived4_low(tri,sides,k1,k2,k3,k4,derived_str));
...
function concyclic_derived_quads(Xks) {
    ...
    const results = quads.map(p=>concyclic_derived_all4(g_tri,sides,p[0],p[1],p[2],p[3]))
    .flat(1)
    .sort((a,b) => (a.tri > b.tri) ? 1 : ((b.tri > a.tri) ? -1 : 0));
    return results;
}

function draw_mounted_locus(n, a, tDeg, locus, clr, locus_type, dr_tri,
 ...
 function draw_mounted_locus_branched(n, a, tDeg, locus_branches, clr, locus_type, dr_tri,
    ...
    if (dr_tri)
       draw_mounted(ons, clr, false, true);
    if (locus_type != 'none')
       draw_locus_branched(locus_branches, ons, n, clr, 0.01, locus_type);
       //draw_locus_only(locus, clr);
 }
 ...
 function draw_billiard_locus_branched(n, a, tDeg, locus_branches, clr, locus_type, dr_tri) {
    let ons = orbit_normals(a, tDeg);
    if (dr_tri)
       draw_orbit(ons, clr, false, true);
    if (locus_type != 'none')
 ...
 function draw_billiard_or_mounted_branched(n, a, tDeg, locus_branches, clr, locus_type, dr_tri, mounting) {
    if (mounting == "billiard") {
       draw_billiard_locus_branched(n, a, tDeg, locus_branches, clr, locus_type, dr_tri);
    } else  {
       draw_mounted_locus_branched(n, a, tDeg, locus_branches, clr, locus_type, dr_tri, mounting);
    }
 }
...
function create_locus_branches(a,tDegStep,tDegMax,trilin_fn,xn_fn) {
 ...
 // no asymptotes
function make_locus_branched(a, n, tDegStep, mounting, locus_type) {
    let trilin_fn = get_fn_any(locus_type, n);
    let locus_array;
    if (mounting == "billiard")
        locus_array = create_locus_branches(a, tDegStep, 180, trilin_fn, get_Xn_orbit);
    else { // non-billiard
        const eps = 0.001;
        let [v1, v2] = getV1V2(a, mounting, eps);
        //let [v3, xn] = get_Xn_mounted(a, 0 + eps, v1, v2, trilin_fn);

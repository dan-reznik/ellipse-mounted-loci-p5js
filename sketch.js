...
function get_fn_any(locus_type, n) {
  if (locus_type.substr(0, 2) == "f_") locus_type = locus_type.substr(2);
  switch (locus_type) {
    case "brocard_1":
      return get_brocard(1);
    case "brocard_2":
      return get_brocard(2);
    default:
      return get_fn_trilin(n);
  }
}
...
function create_locus(locus_type_changed) {
  ...
  if (locus_type_1 != "none" && ["1", "0"].includes(locus_type_changed)) {
    g_locus_Xn1_branched = make_locus_branched(
      a,
      g_ui.Xn1,
      tdegStep,
      g_ui.mounting_Xn1,
  ...
  if (locus_type_2 != "none" && ["2", "0"].includes(locus_type_changed)) {
    g_locus_Xn2_branched = make_locus_branched(
      a,
      g_ui.Xn2,
      tdegStep,
      g_ui.mounting_Xn2,
  ...
  if (locus_type_3 != "none" && ["3", "0"].includes(locus_type_changed)) {
    g_locus_Xn3_branched = make_locus_branched(
      a,
      g_ui.Xn3,
      tdegStep,
      g_ui.mounting_Xn3,
...
function export_PNG() {
  let double_digit = function (myNumber) {
  ...
  document.getElementById("Export_PNG").addEventListener("click", function () {
    var today = new Date();
    var date_time =
      double_digit(today.getDate().toString()) +
      double_digit((today.getMonth() + 1).toString()) +
      today.getFullYear().toString() +
      "_" +
      double_digit(today.getHours().toString()) +
      double_digit(today.getMinutes().toString()) +
      double_digit(today.getSeconds().toString());
    //canvas = document.getElementById('defaultCanvas0');
...
function mouseOverCanvas() {
  ...
  canvas_div.addEventListener("mouseover", function () {
    this.mouseIsOver = true;
  });
  canvas_div.addEventListener("mouseout", function () {
...
function Bbox_onclick(n) {
  document
    .getElementById("Bbox_" + n)
    .addEventListener("click", () => bbox_rescale(n));
}
...
<<<<<<< HEAD
  ...
  if (g_ui[key] !== g_ui_reset[key]) {
    if (key == "a") {
      ...
      return (
        original_to_url_params[key] + "=" + abs(g_ui[key]).toFixed(3) + "&"
      );
    else return original_to_url_params[key] + "=" + g_ui[key] + "&";
  } else return "";
}
  ...
  link_keys.forEach(function (key) {
    if (url_params_to_ui_keys.includes(key)) {
      ...
      if (["a", "Xn1", "Xn2", "Xn3", "animStep0"].includes(ui_key)) {
        ...
        ["ell", "draw_tri_1", "draw_tri_2", "draw_tri_3"].includes(ui_key)
      )
...
>>>>>>> c69c103eb8d5e3da20d85e3faf112c2775325ddb
  ...
  if (g_ui["locus_type_3"] !== "none") {
    ui_changed("3");
    if (!url_change_canvas) bbox_rescale("3");
  }
  if (g_ui["locus_type_2"] !== "none") {
    ui_changed("2");
    if (!url_change_canvas) bbox_rescale("2");
  }
  if (g_ui["locus_type_1"] !== "none") {
    ui_changed("1");
    if (!url_change_canvas) bbox_rescale("1");
  }
}
...
function a_anim() {
  ...
  if (g_ui.a_speed != 0.0) {
    ...
    else if (g_ui.a + g_ui.a_speed <= g_ui.a_min) {
      g_ui.a = g_ui.a_min;
      a_slider.value = g_ui.a;
      a_text.innerHTML = a_slider.value;
      g_ui.a_speed = -g_ui.a_speed;
...
function a_text_input() {
  ...
  var a_max = document.getElementById("a_max");
  a_min.addEventListener("focusout", function () {
    if (+this.value < 1.001) this.value = 1.001;
    else if (+this.value > +a_max.value) this.value = +a_max.value;
    g_ui["a_min"] = this.value;
    ui_changed_type();
  });
  a_min.addEventListener("keydown", function (e) {
    if (e.keyCode == 9) {
      if (this.value == "") this.value = 1.001;
      else {
        if (+this.value < 1.001) this.value = 1.001;
        else if (+this.value > +a_max.value) this.value = +a_max.value;
      }
  ...
  a_max.addEventListener("focusout", function () {
    ...
    else if (+this.value < +a_min.value) this.value = +a_min.value;
...
function export_JSON_onclick() {
  document.getElementById("Export_JSON").addEventListener("click", function () {
    ...
    if (g_ui.locus_type_1 != "none")
      ui_object = {
        ...ui_object,
        ...{ locus1: trunc_locus_xy(g_locus_Xn1_branched, 4) },
      };
    if (g_ui.locus_type_2 != "none")
      ui_object = {
        ...ui_object,
        ...{ locus2: trunc_locus_xy(g_locus_Xn2_branched, 4) },
      };
    if (g_ui.locus_type_3 != "none")
      ui_object = {
        ...ui_object,
        ...{ locus3: trunc_locus_xy(g_locus_Xn3_branched, 4) },
      };
...
function conic_type_onchange(locus_type) {
  ...
  switch (locus_type) {
    case "1":
      conic_type.innerHTML =
        g_ui.locus_type_1 == "none" ? "" : locus_conic(g_locus_Xn1_branched);
      break;
    case "2":
      conic_type.innerHTML =
        g_ui.locus_type_2 == "none" ? "" : locus_conic(g_locus_Xn2_branched);
      break;
    case "3":
      conic_type.innerHTML =
        g_ui.locus_type_3 == "none" ? "" : locus_conic(g_locus_Xn3_branched);
      break;
    default:
...
function setup() {
  ...
  let g_ui_reset = {
    ...
    a_max: 4.0,
    ell: true,
    locus_type_1: "trilins",
    locus_type_2: "none",
    locus_type_3: "none",
    Xn1: 1,
    Xn2: 1,
    Xn3: 1,
    tri_type_1: "reference",
    tri_type_2: "reference",
    tri_type_3: "reference",
    draw_tri_1: true,
    draw_tri_2: false,
    draw_tri_3: false,
...
function draw() {
  ...
  if (g_ui.ell) draw_ellipse(+g_ui.a);
  let stroke_w = sqrt(g_scale / g_scale0) * 0.01;
  draw_billiard_or_mounted_branched(
    g_ui.Xn1,
    +g_ui.a,
    g_tDeg,
    g_locus_Xn1_branched,
  ...
  draw_text_full(
    "(c) 2020 Darlan & Reznik",
    [g_width - 150, g_height - 24],
    clr_blue
  );
  draw_text_full(
    "dan-reznik.github.io/ellipse-mounted-loci-p5js/",
    [g_width - 260, g_height - 10],
    clr_blue
  );
